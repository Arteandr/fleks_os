...
FS::FS(std::string filename, bool with_root) {
  ...
  fd.seekg(this->superblock->s_block_size * 1, std::ios::beg);
...
bitmap *FS::get_inode_bitmap(size_t group_no) {
  ...
  if (!fd.read(buffer, this->superblock->s_block_size)) {
    log("Ошибка при чтении битовой карты inode", LogLevel::error);
    return nullptr;
  }
  bitmap *bm = (bitmap *)malloc(sizeof(bitmap));

  bm = new bitmap(buffer, this->superblock->s_inodes_per_group);
  return bm;
}
...
void FS::remove(const char *filename) {
  ...
  if (i_node->mode(S_SYSTEM)) {
    log("Невозможно удалить системный файл \"" +
            std::string(entry.directory->name) + "\"",
        LogLevel::error);
    delete[] entry.block;
    return;
  }
...
u32 FS::add_user(const char *login, const char *password) {
  ...
  if (this->user_exist(login)) {
    log("Имя пользователя занято", LogLevel::error, true);
    std::cout << std::endl;
    return 0;
  }
